...
translate(0,-160);
    for(int i = 0; i < soilHealth.length; i++){
      ...
      for(int s=0 ; s<8 ; i++){//layer1 to 8
      image(stones[0][j],80,180);
      soilHealth[s][s]=30;
    }
    for(int s=0;s<8;i++){//layer9-16
      for(int a=8;a<16;a++){
        if( a==8 || a==11 || a==12 || a==15 ){
          if( a==1 || a==2 || a==5 || a==6 ){
          image(stones[0][j] , s*80 , 160+a*80);
          }
        }else{
          if(s==0 || s==3 ||s==4 || s==7){
          image(stones[0][j] ,s*80 ,160+a*80);
    ...
    for(int s=0 ; s<8 ; s++){//layer17-24
      for(int a=16 ; a<24 ; a++){
        if(s+a==17||s+a==18||s+a==20||s+a==21||s+a==23||s+a==24||s+a==26||s+a==27||s+a==29||s+a==30){
          image(stones[0][j],s*80,160+a*80);
          if(s+a==18||s+a==21||s+a==24||s+a==27||s+a==30){
          image(stones[i][j],s*80,160+a*80);
          }
        }
      }
    }
